...
def draw_figure6(input_dir):
    timelimit = EXP_ENV["TIMELIMTS"]["figure6"] if "original" not in input_dir else 86400
    ...
    for tool in tool_dict:
        ax1.plot(tool_dict[tool]["10"],
                 label=tool,
                 marker=markers[tool],
                 linestyle='-',
                 color=color[tool])
        ax2.plot(tool_dict[tool]["20"],
                 label=tool,
                 marker=markers[tool],
                 linestyle='-',
                 color=color[tool])
        ax3.plot(tool_dict[tool]["40"],
                 label=tool,
                 marker=markers[tool],
                 linestyle='-',
                 color=color[tool])

    plt.ylim([0, min(max_10 * (11 / 10), timelimit)])
    ...
    ax1.set_ylim(0, min(max_10 * (12 / 10), timelimit))
    ax2.set_ylim(0, min(max_2040 * (12 / 10), timelimit))
    ax3.set_ylim(0, min(max_2040 * (12 / 10), timelimit))
...
def draw_cactus_plot(input_dir):
    ...
    for tool in tools:
        iters = [
            int(x) for x in list(df[tool])
            if int(x) < timelimit
        ]
        iters.sort()
        all_ttes += iters
        global_med = np.median(iters)
        tool_dict[tool] = {'ttes': iters}
    ...
    x_ticks = [0, 20, 40, 60, 80, 100, 120, 140,
               160]  # Set the positions of the x-axis ticks
    ...
    plt.ylim([0, min(global_max, timelimit)])
    plt.tight_layout()
